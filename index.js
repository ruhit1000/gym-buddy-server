...
app.get("/api/classes/manage", verifyToken, verifyAdmin, async (req, res) => {
  try {
...
app.get("/api/forum", async (req, res) => {
  try {
    ...
    res.status(500).send({
      success: false,
      message: "Internal server error while fetching community forum posts.",
    });
  }
});
...
app.post("/api/forum", verifyToken, async (req, res) => {
  try {
    ...
    res.status(500).send({
      success: false,
      message: "Internal server error while compiling forum post submission.",
    });
  }
});
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.error("Error processing trainer application submission:", error);
      res.status(500).send({
        success: false,
        message:
...
app.post("/api/favorites/toggle", verifyToken, async (req, res) => {
  try {
    ...
    const existingFavorite = await favoritesCollection.findOneAndDelete(query);
...
app.get("/api/favorites/my-favorites", verifyToken, async (req, res) => {
  try {
    ...
    res.status(500).send({
      success: false,
      message: "Internal server error while retrieving aggregated favorites.",
    });
  }
});
...
app.get("/api/forum/:id", async (req, res) => {
  try {
    ...
    const [post, comments] = await Promise.all([
      ...
      commentsCollection.find(commentsQuery).sort({ createdAt: 1 }).toArray(),
    ]);
    ...
    res.status(500).send({
      success: false,
      message: "Internal server error while loading discussion thread details.",
    });
  }
});
...
app.patch("/api/forum/comments/:commentId", verifyToken, async (req, res) => {
  try {
...
app.delete("/api/forum/comments/:commentId", verifyToken, async (req, res) => {
  try {
    const { commentId } = req.params;
    const userId = req.user._id;
...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).send({
        success: false,
        message: "Internal server error during administrative post deletion.",
      });
    }
  },
);
...
app.patch("/api/users/manage", verifyToken, verifyAdmin, async (req, res) => {
  try {
...
app.get("/api/applied-trainers", verifyToken, verifyAdmin, async (req, res) => {
  try {
    const query = {
  ...
  async (req, res) => {
    try {
      ...
      if (result.modifiedCount === 1) {
        ...
        res.status(404).send({
          success: false,
          message: "Target user application not found or status already set.",
        });
      }
    } catch (error) {
